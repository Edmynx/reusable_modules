...
int32_t main() {
    ...
    */
    ...
    /*
     * Now, hash an object into the existing but empty hashtable
     */
    result_int = hput(htp, car1, car1->plate, sizeof(car1->plate));
    if(result_car) {
        fprintf(stderr, "Test Error: 'hput()' should be able to hash object into an existing but empty hashtable\n");
        free(car1);
        free(car2);
        free(car3);
        exit(EXIT_FAILURE);
    ...
    /*
     * Now, hash another object into the existing and non-empty hashtable
     */
    result_int = hput(htp, car2, car2->plate, sizeof(car2->plate));
    if(result_car) {
        fprintf(stderr, "Test Error: 'hput()' should be able to hash object into an existing and non-empty hashtable\n");
        free(car1);
        free(car2);
        free(car3);
        exit(EXIT_FAILURE);
