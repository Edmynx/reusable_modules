/*
 * hremove.c -- tests hremove() function in hash.c
 */
...
int32_t main() {
    ...
    /*
     * Remove from a non-existing hashtable
     */
    result_car = hremove(htp, compare_plate, car1->plate, sizeof(car1->plate));
    if(result_car) {
    ...
    /*
     * Now, remove from the existing but empty hashtable
     */
    result_car = hremove(htp, compare_plate, car1->plate, sizeof(car1->plate));
    if(result_car) {
        fprintf(stderr, "Test Error: 'hremove()' is not supposed to remove an object from an empty hashtable\n");
        free(car1);
        free(car2);
        free(car3);
        exit(EXIT_FAILURE);
    ...
    /*
     * Then, remove from the existing and non-empty hashtable
     */
    result_car = hremove(htp, compare_plate, car1->plate, sizeof(car1->plate));
    if(!result_car) {
        fprintf(stderr, "Test Error: 'hremove()' should be able to remove car1 from hashtable\n");
        free(car1);
        free(car2);
        free(car3);
        exit(EXIT_FAILURE);
